...
However, do note that there is some space put before the \verb|&| alignment position.
If you want to collect several aligned equations inside a brace,
it might be nicer to fake the output with help from the \env{aligned} environment
...
\ShowExample
%
Here the \cmd{left}--\cmd{right} pair is used to produce a correctly sized brace.
...
\future{mathtools offers cramped versions}
...
\subsection{The common way with \pkg{amsthm}}\label{sec:amsthm}
...
Just one example: with \pkg{amsthm} you are supposed to renew \cmd{qedsymbol},
whereas with \pkg[causing trouble]{ntheorem} you are supposed to call it.}

A much more compatible way to make your theorems slightly fancier is the \pkg{thmtools} package.
...
\begin{gotcha}
As of June~2025, there seems to be a bug where hyperlink targets can sometimes be wrong:
a link to Lemma~3.4 might lead to Lemma~1.4,
because only the ``.4'' part is recorded.
This can be worked around by passing \verb|hypertexnames=false| to \pkg{hyperref},
either as a package option or via \cmd{hypersetup}.
\end{gotcha}

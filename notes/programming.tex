...
\filetype{idx} Index created by the \prg{makeindex} program.
    \todo{Reference if makeindex is mentioned}
...
The prime example is the \prg{bibtex} program.
...
This might be extended to theses, and lecture notes
...
\todo{Check whether there was something more}

There is also a command \cmd{includeonly} for temporarily restricting to a subset of the included files.
The same can be achieved by selectively commenting out input commands.
...
-- consisting only of the preamble and input commands.
\end{practices}
...
\todo{The standalone document class}
...
\todo{How to set the main file with Overleaf}
...
If this happens inside a TikZ picture, you have very likely mistyped a command.
...
\todo{When to create a style file?}

\todo{Basic syntax}

\todo{Requiring packages}
...
\todo{Using counterwithin to customize standard counters}
...
%
%
\section{\emph{Further topics*}}\label{sec:if}

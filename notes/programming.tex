...
If you want to add a table of contents, you just add \cmd{tableofcontents} in a suitable spot.
...
We talk about the layout algorithm in \Cref{sec:layout}.

As \TeX{} never backtracks,
...
The trick is simple:
Whenever a \cmd{section} or \cmd{label} command is encountered,
...
Code examples are written inside a special environment within the chapter \verb|.tex| file.
\TeX{} outputs the example to a temporary file so that it can be read back \emph{twice}:
once to print out the code, and then again to show the results.
\end{technote}
...
and the Beamer chapter where several \texttt{\textbackslash documentclass} commands appear.
It tries to compile the chapters as standalone files, failing spectacularly with 1000+ errors.}
...
\begin{gotcha}
\cmd{include} produces only a warning and not an error if it cannot find the specified file.
...
\begin{gotcha}
If you define commands, counters, or float types inside \cmd[gotchas]{include}'d files,
you will probably end up in pain whenever you use \cmd{includeonly}.
Define those things in the preamble.\footnotemark
\end{gotcha}
\footnotetext{Again an observation sponsored by these notes,
since there are examples defining all of those objects.}
...
There is also a \pkg{standalone} document class.
...
it instead provides the \cmd{includestandalone} command.
This can be used to include standalone .tex files like with \cmd{input}.
...
\begin{itemize}
    \item If \verb|complicated.pdf| exists,
        it includes it with \cmd{includegraphics}.
...
This can be useful for reducing compile times on complicated TikZ pictures,
or a very specialized workflow:
Some TikZ extension packages require \prg{lualatex},
which is not supported by arXiv or many journals.
With this trick, you can use \prg{lualatex} for your writing flow,
...
\begin{itemize}
\item \pkg{babel} should be loaded very early,
\item \pkg{cleveref} must be loaded after \pkg{hyperref} and theorem packages.
\end{itemize}
\end{gotcha}
...
\LaTeX{} keeps a lot of internal state in integer variables called \emph{counters}\index{counters}.
...
you can use the starred \cmd{counterwithin*} form, or renew the format.
...
The \cmd{label} command creates a label referring to the ``latest'' updated counter.
The usual \cmd{stepcounter} command does not update this tag;
instead, the \LaTeX{} sectioning commands etc.\ use the \cmd{refstepcounter} command.

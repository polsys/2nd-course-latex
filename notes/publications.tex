...
The first version of these notes advocated for the \pkg{cleveref} package,
...
In this case a patch was added to the kernel to fix the issue,
but this does not fix the underlying incompatibility.

My recommendation is to replace \pkg{cleveref} with \pkg{zref-clever} in new projects.
One can define workaround commands (see below) to make the syntax similar.
\end{warning}
...
It also contains some advanced features missing from \pkg{cleveref}.

This package adds a \cmd{zcref} command
that does exactly what you would expect:
you write \verb|\zcref{thm:pythagoras}|,
...
In the beginning of sentence, you can write \verb|\zcref[S]{...}|.
The optional argument tells the package to capitalize and not abbreviate the first reference.
In \pkg{cleveref} the operative command is \cmd{cref} or \verb|\Cref|,
depending on capitalization.

Reference style can be customized by quite a lot
with the \cmd{zcsetup} command.
For example, \verb|\zcsetup{cap}| would capitalize all references, which is often desired.
These can also be passed as an optional argument to \cmd{zcref}.
See the package documentation for all options.

A nice feature of these packages is that you can also pass several references,
and they sort out how to present them.
That is,
\begin{ExampleCode}
...
\begin{practices}
If you have used \pkg{cleveref}, you may be able to convert to \pkg{zref-clever} by something like
\begin{ExampleCode}
% Old

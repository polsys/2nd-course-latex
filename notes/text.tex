...
This is called a \emph{ligature}.\index{ligatures}
...
One notable example is \pkg{fontawesome6},
...
The inclusion of emoji will be discussed below in \Cref{sec:emoji}.
...
\item[Series] This is the combination of weight and width of the font.
    ...
    the methods in \Cref{sec:typefaces} can create more series.
...
They are easier to parse for automatic tools like \prg{latexdiff},
and moreover \cmd{textit} does a tiny typographic tweak called \emph{italic correction}
at the interface between italic and upright styles:\index{italic correction}
%
\begin{VerbatimOut}{\jobname.tmp}
\textit{Super}charged\\
...
One neat use of verbatim environments is to comment out large blocks of text.
\index{comment!large block}%
This example requires \pkg{fancyvrb} and its capitalized \env{Verbatim} environment:
%
\begin{VerbatimOut}{\jobname.tmp}
Some text.
...
\ShowExample
%
This can be further simplified with the \cmd{DefineVerbatimEnvironment} command,
which creates a customized environment based on \env{Verbatim}:
%
\begin{VerbatimOut}{\jobname.tmp}
...
you should go to \Cref{sec:layout}.
...
you can also use the \cmd{par} command to cause a paragraph break.
...
again look at \Cref{sec:layout} and specifically \Cref{sec:paragraph layout}.
If you need to suppress indentation of a single paragraph,
you can use \cmd{noindent} at the beginning.
%
\begin{VerbatimOut}{\jobname.tmp}
...
\index{color!blending}\label{ex:color blending}
...
(but see \Cref{sec:table footnotes} for more on this!).
...
and they are used to build a PDF navigation bar (when \pkg[PDF navigation]{hyperref} is loaded).
\end{practices}
...
The \cmd{appendix} command resets the top-level numbering
...
\cmd{frontmatter}, \cmd{mainmatter}, and \cmd{backmatter} commands.
...
The \verb|\frontmatter| command should appear immediately after \verb|\begin{document}|.
...
To customize heading styles, it is easiest to use the \pkg{titlesec} package.
It provides two ways to customize headings: a ``simple'' and an ``advanced'' interface.

\begin{gotcha}
If you use the \pkg[customizing headings]{memoir} or \pkg[customizing headings]{koma-script}
document classes, this package is not usable.
The document classes provide their own mechanisms instead.
\end{gotcha}

The simple interface is used with the \cmd{titleformat*} command.
...
It uses the unstarred \cmd{titleformat} command,
...
\item Formatting of the heading number.
    ...
    you can get the correct string with \cmd{chaptertitlename}.
...
To center or right-align text, you should use \cmd{filcenter} and \cmd{filouter}/\cmd{filinner}
instead of the standard commands.
These special commands respect the separation parameters.

Let us explore these through a couple examples.
Many more possibilities are shown in the package documentation.

In the next example, chapter titles are centered with \cmd{filcenter}.
The chapter number is stated as an ordinal with the \pkg{fmtcount} package.
...
The \cmd{wordsep} command gives the natural inter-word distance for the font,
...
\end{ExampleCode}
%
\EmbedPdfPage{examples/titlesec.pdf}{1}

The \cmd{titlespacing} command adjusts the spacing around the title.
...
\end{ExampleCode}
%
\EmbedPdfPage{examples/titlesec.pdf}{2}

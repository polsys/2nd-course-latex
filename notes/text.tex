...
Due to historical and technical reasons, \TeX{} does not natively support colors.
The \pkg{xcolor} package adds this support.
Many other packages load \pkg{xcolor} automatically
and support the color syntax (for example \pkg{tikz}).
...
Like with the font commands, there are two forms of the color specification:
\cmd{textcolor} that colors only its (second) argument,
and \cmd{color} that colors everything until the current scope ends.
...
\item[By name] \verb|\textcolor{blue}{...}| prints the text in \textcolor{blue}{blue}.
    ...
    you get a lot more colors such as \textcolor{DarkOrange}{\texttt{DarkOrange}}.
...
The package also offers a lot of ways to mix colors together.
...
\footnote{Another reason is that the result depends on the specific color model.
Taking averages of RGB values can yield a very different result to averaging HSB values.
There is no `true' way to do this;
in fact, trying to understand what color \emph{really is}
...
It is possible to define custom color names with \cmd{definecolor}.
...
and define the brand color of the Faculty of Science:
%
\begin{VerbatimOut}{\jobname.tmp}
...
\begin{practices}
Never use color as the sole means of indicating information.
Many people read papers in black-and-white prints
or on ebook reader with grayscale screen.
Additionally about 1 in 12 males is red-green colorblind,
the most common form of color vision deficiency.

Even then, pay attention to the color contrast.
